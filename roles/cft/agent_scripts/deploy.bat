...
echo.
echo  --- ÑÎÕÐÀÍÅÍÈÅ PATCH.ZIP --- %r1% - %r2%
echo.
...
echo.
echo  --- ÑÎÕÐÀÍÅÍÈÅ PATCH.ZIP --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\licence.sql
echo  --- ÏÐÎÂÅÐÊÀ ÍÀËÈ×Èß ÄÅÉÑÒÂÓÞÙÅÉ ËÈÖÅÍÇÈÈ ÍÀ ÄÈÑÒÐÈÁÓÒÈÂÍÓÞ ×ÀÑÒÜ ÖÔÒ --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\fio.sql
echo  --- ÏÐÎÂÅÐÊÀ ÐÀÁÎÒÎÑÏÎÑÎÁÍÎÑÒÈ FIO --- êîä âîçâðàòà %ERRORLEVEL%
...
echo.
echo  --- DEPLOY1 ÈÇ ÄÈÑÒÐÈÁÓÒÈÂÍÛÕ ÎÁÍÎÂËÅÍÈÉ ---
echo.
echo ïîäêëþ÷åíèå ê ñåòåâîé ïàïêå \\fsoffice.nsd.ru\depo
...
cd %last_dir%
echo.
echo  --- DEPLOY1 ÈÇ ÄÈÑÒÐÈÁÓÒÈÂÍÛÕ ÎÁÍÎÂËÅÍÈÉ --- êîä âîçâðàòà %DD_LVL%
...
echo.
echo  --- DEPLOY1 PATCH.ZIP --- êîä âîçâðàòà %ERRORLEVEL%
if not %ERRORLEVEL% equ 0 if not exist %conv_file% goto end
set ERRLEV=%ERRORLEVEL%

echo.
echo  --- ÓÄÀËÅÍÈÅ ÝËÅÌÅÍÒÎÂ ---
echo.
...
--launcher.suppressErrors
echo.
echo  --- ÓÄÀËÅÍÈÅ ÝËÅÌÅÍÒÎÂ --- êîä âîçâðàòà %ERRORLEVEL% (èãíîðèðóåòñÿ)

if not exist %fio_dir% goto conv
echo.
echo  --- ÊÎÏÈÐÎÂÀÍÈÅ ÔÀÉËÎÂ ÈÌÏÎÐÒÀ ÍÀ ÑÅÐÂÅÐ ---
echo.
cscript //B C:\eclipse\oxch.vbs %srv% %pwd% %files_dir% %fio_dir%
type %files_dir%\oxch.log
echo.
echo  --- ÊÎÏÈÐÎÂÀÍÈÅ ÔÀÉËÎÂ ÈÌÏÎÐÒÀ ÍÀ ÑÅÐÂÅÐ --- çàâåðøåíî

:conv
if not exist %conv_file% goto ver
echo.
echo  --- ÂÛÏÎËÍÅÍÈÅ ÎÏÅÐÀÖÈÉ ÊÎÍÂÅÐÒÀÖÈÈ ---
echo.
...
echo.
echo  --- ÂÛÏÎËÍÅÍÈÅ ÎÏÅÐÀÖÈÉ ÊÎÍÂÅÐÒÀÖÈÈ --- çàâåðøåíî

if not %DD_LVL% equ 0 (
set DD_LVL=0
echo.
echo  --- DEPLOY2 ÈÇ ÄÈÑÒÐÈÁÓÒÈÂÍÛÕ ÎÁÍÎÂËÅÍÈÉ ---
echo.
cd %distr_dir%
call inst_distr.bat %srv% %pwd%
cd %last_dir%
echo.
echo  --- DEPLOY2 ÈÇ ÄÈÑÒÐÈÁÓÒÈÂÍÛÕ ÎÁÍÎÂËÅÍÈÉ --- êîä âîçâðàòà %DD_LVL%
set ERRORLEVEL=%DD_LVL%
if not %ERRORLEVEL% equ 0 goto end
set DD_LVL=1
)
...
--launcher.suppressErrors
echo.
echo  --- DEPLOY2 PATCH.ZIP --- êîä âîçâðàòà %ERRORLEVEL%
if not %ERRORLEVEL% equ 0 goto end
)

:ver
echo.
echo  --- ÓÑÒÀÍÎÂÊÀ ÍÎÌÅÐÀ ÂÅÐÑÈÈ --- %build_no%
...
echo  --- ÏÐÎÂÅÐÊÈ ÏÎÑËÅ ÓÑÒÀÍÎÂÊÈ ---
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\xmlparser.sql
echo  --- ÏÐÎÂÅÐÊÀ ÐÀÁÎÒÎÑÏÎÑÎÁÍÎÑÒÈ ÁÈÁËÈÎÒÅÊÈ XMLPARSER --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\webpdf.sql
echo  --- ÏÐÎÂÅÐÊÀ ÄÎÑÒÓÏÍÎÑÒÈ ÑÅÐÂÈÑÀ WEBPDF --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\gis_gmp.sql
echo  --- ÏÐÎÂÅÐÊÀ ÐÀÁÎÒÎÑÏÎÑÎÁÍÎÑÒÈ ÎÁÐÀÁÎÒ×ÈÊÎÂ ÑÎÎÁÙÅÍÈÉ ÃÈÑ ÃÌÏ --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\fns.sql
echo  --- ÏÐÎÂÅÐÊÀ ÐÀÁÎÒÎÑÏÎÑÎÁÍÎÑÒÈ ÎÁÐÀÁÎÒ×ÈÊÎÂ ÑÎÎÁÙÅÍÈÉ ÔÍÑ --- êîä âîçâðàòà %ERRORLEVEL%

if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\soa.sql
echo  --- ÏÐÎÂÅÐÊÀ ÎÒÑÓÒÑÒÂÈß ÇÀÂÈÑØÈÕ ÑÎÎÁÙÅÍÈÉ Â Î×ÅÐÅÄßÕ ØÈÍÛ --- êîä âîçâðàòà %ERRORLEVEL%

if %srv:~0,2% equ t6 goto skip1
if %srv:~0,3% equ dev goto skip1
if not %ERRORLEVEL% equ 0 goto end
sqlplus ibs/%pwd%@%srv% @c:\eclipse\cft-server-checks\reportsever.sql
echo  --- ÏÐÎÂÅÐÊÀ ÍÀËÈ×Èß ÑÅÑÑÈÉ ÎÒ ÑÅÐÂÅÐÀ ÎÒ×ÅÒÎÂ --- êîä âîçâðàòà %ERRORLEVEL%
:skip1

if not %ERRORLEVEL% equ 0 goto end
echo  --- ÏÐÎÂÅÐÊÈ ÏÎÑËÅ ÓÑÒÀÍÎÂÊÈ ÓÑÏÅØÍÎ ÇÀÂÅÐØÅÍÛ ---
